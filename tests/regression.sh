...
test_dog_mark_rule_read_failure_is_not_reported_ok() {
    ...
    dog_port_mark_comment() { printf 'dog-port-3200\n'; }
    dog_port_spec_bounds() { printf '3200 3210\n'; }
    # nft 读取成功但输出为空属于查询失败：jq 对空输入以 0 退出，
    # 不能据此把 Dog mark 规则判为“完整”，必须按查询错误上报。
    nft() { :; }
    status=0
    dog_port_mark_rules_complete 3200 1 >/dev/null 2>&1 || status=$?
    [ "$status" -eq 2 ]
}
...
run_test 'Dog mark rule read failures are not reported as complete' test_dog_mark_rule_read_failure_is_not_reported_ok
