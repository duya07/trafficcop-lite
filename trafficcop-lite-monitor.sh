...
check_and_install_packages() {
    local required_commands=("vnstat" "jq" "bc" "ip" "tc" "crontab" "flock" "curl" "tac" "pgrep" "pkill")
    local command_name
...
acquire_tc_hierarchy_lock() {
    ...
    # 共享锁位于世界可写的 /run/lock：拒绝符号链接并用 <> 打开（不截断），
    # 避免本地非特权用户预置符号链接后让 root 截断任意文件。
    if [ -L "$TC_HIERARCHY_LOCK_FILE" ]; then
        echo "共享 TC 锁文件被替换为符号链接，已拒绝: $TC_HIERARCHY_LOCK_FILE" >&2
        return 1
    fi
    exec 6<>"$TC_HIERARCHY_LOCK_FILE" || return 1
    if [ ! -f "$TC_HIERARCHY_LOCK_FILE" ] || [ "$(stat -c %u "$TC_HIERARCHY_LOCK_FILE" 2>/dev/null)" != "0" ]; then
        exec 6>&-
        echo "共享 TC 锁文件非 root 所有或不是普通文件，已拒绝: $TC_HIERARCHY_LOCK_FILE" >&2
        return 1
    fi
